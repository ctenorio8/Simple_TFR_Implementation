...
def autoWindowBruteForce(timeVector, signalArray, lossFunc, *searchArgs):
    ...
    **Returns:**
    ...
    nPoints, nSignals = np.shape(signalArray)
    for signalNum in range(nSignals):
        t0_current, lossMin = lossFunc(timeVector, utils.toneBurstLoss_t0,
...
###########################################################################################
# Empirical Mode Decomposition (Sifting Methods)
###########################################################################################

# TODO: Read through EMD package workflow to see if this would be useful for our low-frequency stuff

# def nde_sift(signal: np.ndarray,
#              ):
#